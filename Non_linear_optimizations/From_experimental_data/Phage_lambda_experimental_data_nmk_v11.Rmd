...
# ggsave("Fit_experimental_data_B80.tiff", width=4, height=9, dpi=1000)
```

```{r, eval=FALSE, fig.width = 4, fig.height = 9}
ggarrange(
  Fig_logit_prev.transparent + geom_line(data=best_simul, aes(group=prev0), cex = 1) +
    ylab("Logit-prévalence") +
    scale_color_manual(values=c('#DA0F0F', '#0E5DD6'), labels=c("épidémique", "endémique")) +
    theme(axis.title.x = element_blank(), legend.title = element_blank()),
  Fig_logit_g.transparent + geom_line(data=best_simul, aes(group=prev0), cex = 1) +
    ylab("Logit-fréquence infectée par le phage virulent") +
    theme(axis.title.x = element_blank(), legend.title = element_blank()),
  Fig_logit_q.transparent + geom_line(data=best_simul, aes(group=prev0), cex = 1) +
    ylab("Logit-fréquence des virions virulents") +
    xlab("Temps (heure)") +
    theme(legend.title = element_blank()),
  ncol=1, common.legend=TRUE, align='v', labels=LETTERS[1:3],
  label.x=0.96, label.y=0.96, font.label=list(size=13), hjust=1)
...
plot_grid(Fig_relvar_a, Fig_relvar_r, ncol=1, labels=LETTERS[1:2], align = 'v')
```

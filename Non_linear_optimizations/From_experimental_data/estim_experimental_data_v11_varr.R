...
parms0 <- parms0[,-which(colnames(parms0)=="r")]
...
for(r_var in 1:5){
  ...
  estim_df <- Estim_nmk_parallel(parms0=parms0, constants=c("alpha_w"=estim_alpha_exp[["alpha_w"]],
                                 ...
                                 maxfeval=4000, Ncores=8)
  
  write.csv(estim_df, paste0("Outputs_csv/Estim_nmk_parallel_experimental_data_nstarts", n_starts, "_r", r_var, "_v11.csv"), row.names = FALSE)
}
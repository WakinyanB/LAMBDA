...
simul_data_FACS <- read.csv("Data/Simulated_data/simul_data_FACS_2.csv", header = TRUE)
simul_data_qPCR <- read.csv("Data/Simulated_data/simul_data_qPCR_2.csv", header = TRUE)
...
estim_df <- Estim_nmk_parallel(parms0 = parms0, constants = c("alpha_w"=estim_alpha_df$alpha_w,
                               ...
                               maxfeval = 8000, Ncores = 15)
Sys.time()-t0

write.csv(estim_df, "Outputs_csv/Estim_nmk_parallel_simulated_data_sd_noise_05_timestep_01_n5x2000_seed123_v11.csv", row.names = FALSE)